...
/* Xgboost v3
 * flattened score equation: instead of f = f1+f2+f3+f4  simplified version is used
 *
 * */
...
void calculateTree(CryptoContext<DCRTPoly> &cc, vector<Ciphertext<DCRTPoly>> &res, vector<Ciphertext<DCRTPoly>> &z1, vector<Ciphertext<DCRTPoly>> &z2,
    ...
    Plaintext c1 = cc->MakeCKKSPackedPlaintext(leaves[0]); //(l1-l2)
    Plaintext c2 = cc->MakeCKKSPackedPlaintext(leaves[1]); //(l2-l4)
    Plaintext c3 = cc->MakeCKKSPackedPlaintext(leaves[2]); //(l4-l3)
    Plaintext c4 = cc->MakeCKKSPackedPlaintext(leaves[3]); //l4

    //f =  z1 * z2 * (l1-l2) + z1 * (l2 - l4) + (z1 - 1) * z3 * (l4-l3) +l4
#pragma omp parallel for
    for (usint i = 0; i < res.size(); ++i) {
        ...
        temp = cc->EvalAdd(temp,c2); // depth 1
        ...
        res[i] = cc->EvalAdd(res[i], c4); // depth 1
...
int main() {
        ...
        CryptoContext<DCRTPoly> cc =
                CryptoContextFactory<DCRTPoly>::genCryptoContextCKKS(4, 40, 1408, HEStd_128_classic);
